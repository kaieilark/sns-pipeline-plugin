...
**利用者がSNS運用の専門用語（テーマ・バックボーン等）を1つも知らない前提で、対話形式・手厚いサポートで進める**（engine/setup_prompt.md §0-1 対話の進め方・用語集に従う。config.json のキー名をそのまま尋ねない）。

手順:
1. `"$SNS_PLUGIN_ROOT/engine/setup_prompt.md"` を読み、その手順に従う。**まず §0-1（対話の進め方・用語集）を読んでから着手する。**
2. 前提確認（テナント自身の Lark Custom App / lark-cli プロファイル / 対象Baseとbot招待）→ config雛形コピー → **画像生成バックエンドの自動設定（下記・確認不要）** → テーママスター・参考元テーブル作成（§2） → テーマ作成（/sns-theme の追加を必要数） → バックボーンストック（任意・§3-2） → 素材ライブラリ（任意・§3-3） → 拡張機能導入 → 定期実行 → 動作確認。
