...
- **専門用語を知らなくても対話形式で設定できる**。「テーマ」「バックボーン」等の用語は、尋ねる前に平易な言葉でAIが説明してから聞く設計（1つずつ・具体例つき・任意機能は無理に埋めさせない）。SNS運用の初心者でも `/sns-setup` を実行するだけで一通り設定できる。
- **テーマを複数運用**できる（例:「会社ブランディング用」「募集用」）。テーマは設定に足すだけで増やせる。
- **投稿（記事）はテーマごとに別テーブル**に分けて管理。素材案はチャットIDで振り分くので全テーマ共通の1テーブルでよい。
- **テーマは正規化されたマスターテーブルで管理**（テーママスター1つ＋「テーマリンク」＋formula）。同じ選択肢を複数テーブルに重複させないため、テーマの追加・改名はマスター側の1レコードを直すだけで済む。
- **バックボーン（背景・思想）**をテーマごとに設定。HPのURL / 直接入力 / 資料ファイル から作れ、後からいつでも変更可能。
- **バックボーンストック（継続蓄積・任意機能）**: 特定のチャットに随時書き溜める思い・方針・エピソードを、テーマを問わず本文作成のレンズとして自動で参照し続ける。
- **画像テイスト**をテーマごとに設定。Instagram等のURL / 画像添付 / 説明文 で指定でき、後から変更可能。
- **画像の骨格を直近10枚で重複させない**（毎回レイアウトの型を変える）。テーマ別にローテーション管理。枚数も1〜5枚を内容量で判断（固定既定なし）。
- 記事は「読み手の教育 → 問い合わせへの誘導」方針。**本文作成前にそのテーマの公開済み投稿と矛盾しないか自動チェック**。画像は画像生成モデルのみで文字・ロゴまで描く。
- **取込元チャットが無くても、対話で作成〜Webhook発行までガイドし、その場で設定に登録する**（テーマ追加時・バックボーンストック設定時）。
- **画像生成バックエンドを選べる**: Claude Code自身は画像生成モデルを持たないため、セットアップ時に「Codex CLIのサブスクリプションを使う／OpenAI API等の従量課金APIを使う／自作スクリプトを使う」から選ぶ。Codex自身に導入した場合は確認なしで自身のimage_genを自動選択。
- **素材ライブラリ（任意機能）**: 画像生成に使い回す既定の参照画像（店舗オーナー写真・ロゴ等）をBaseで管理。使用フラグONの素材だけが参照候補になり、備考の説明が画像生成プロンプトにそのまま反映される。
