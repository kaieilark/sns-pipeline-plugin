...
4. **投稿作成**: `"${CLAUDE_PLUGIN_ROOT}/engine/compose_prompt.md"` を読み、その手順（絶対ルール含む）を忠実に実行する。
   ...
   - 画像の実際の生成方法は `config.json` の `IMAGE_BACKEND` に従う（`codex_native`/`codex_cli`/`openai_api`/`custom_script`。未設定なら `sns-setup` の §0-6 が未完了なので先に案内する）。`MATERIALS_TABLE_ID` が設定されていれば、使用フラグONの素材ライブラリ画像も参照候補に含める。
