...
新しいテナントでこのパイプラインを立ち上げる。**利用者がSNS運用の専門用語（テーマ・バックボーン等）を1つも知らない前提で、対話形式・手厚いサポートで進める**（`engine/setup_prompt.md` §0-1 対話の進め方・用語集に従う。config.json のキー名をそのまま尋ねない）。

## 手順
1. `"${CLAUDE_PLUGIN_ROOT}/engine/setup_prompt.md"` を読み、その手順に従う。**まず §0-1（対話の進め方・用語集）を読んでから着手する。**
