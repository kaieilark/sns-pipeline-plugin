...
> **「テーマ」「バックボーン」などの言葉を初めて聞く方でも大丈夫です。** `/sns-setup` を実行すると、AIエージェントが専門用語を1つずつ平易な言葉で説明しながら、対話形式で必要な項目を聞いていきます（`engine/setup_prompt.md` §0-1）。以下は内部的に何が起きているかの技術的な説明です。実際の対話ではAIが噛み砕いて質問してくれるので、この文書の用語を覚える必要はありません。
